...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // Mapping from token ID to buyers address
    mapping (uint256 => address) private _orders;
    
    // Mapping from token ID to bool (for sell or not);
    mapping (uint256 => bool) private _forSell;
    ...
    function approveOrder(address from, uint256 tokenId) public {
        require(_prices[tokenId] != 0, "ERC721: price does not exist" );
        require(_orders[tokenId] != address(0), "ERC721: order does not exist" );
        payable(ERC721.ownerOf(tokenId)).sendValue(_prices[tokenId]);
        safeTransferFrom(from, _orders[tokenId], tokenId);
    }
    
    function order(address to, uint256 tokenId) public payable {
        require(_exists(tokenId), "ERC721: operator query for nonexistent token");
        require(_forSell[tokenId] == true, "ERC721: token is not for sell");
        require(_prices[tokenId] != 0, "ERC721: price does not exist" );
    ...
    function setPrice(address from, uint256 tokenId, uint256 price) public {
        require(price > 0, "ERC721: price cannot be less than or equal to zero");
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        ...
        // Clear orders from the previous owner
        _orders[tokenId] = address(0);
        
        // Reset token to not for sell
        _forSell[tokenId] = false;
